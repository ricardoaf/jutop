function BARS = GenerateGS (NODE,ELEM,Lvl,ColTol)
if nargin<4 || isempty(ColTol), ColTol=0.9999; end

% Get element connectivity matrix
Nn = size(NODE,1); Ne = length(ELEM);
A1 = sparse(Nn,Nn);
...
for i=2:Lvl
    ...
    % RemoveFlag = []; % considering no domain restrictions for now
